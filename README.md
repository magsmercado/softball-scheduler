...
- **Schedule & Bags** -- every week's matchups, which field each game is
  on, and which numbered bag the home team is carrying.
- **Off-Day Bag Transfers** -- the (hopefully short) list of "Team X hands
  Bag N to Team Y before this week" instructions. Ideally this list is
  short or empty most weeks.
- **Bag Tracker (by bag #)** -- each bag's full season itinerary, if you
  want to trace one bag's whole path.
- **Field Fairness** -- each team's total low-field vs. good-field game
...
The real limit isn't team count, it's *simultaneous games*: fields 9-15
...
it in a banner up top so you can adjust by hand -- swap two weeks, add an
...
- **Byes go to blacked-out teams first.** For each calendar date, the app
  ...
  gives them first claim on that week's bye slots -- so a team's blackout
...
One tradeoff worth knowing: a team with a lot of blackout dates will
naturally rack up more total byes than a team with none, since their
blackout dates keep claiming bye slots ahead of the fairness rule. That's
intentional -- avoiding the blackout takes priority -- but it means "byes
spread evenly" is a secondary goal, not a guarantee, for teams with heavy
blackout schedules.
...
matchup pairs a bag-holder against a non-holder -- no extra effort needed,
the bag just goes home with whichever of the two teams needs it for their
next game.
...
facing each other). That's the only time an off-day transfer is genuinely
needed: one bag moves from the surplus team to the short team before game
...
With zero or one bye per week (the classic case), this produced **zero**
off-day transfers across an entire season in testing. With two or more
teams on bye every week, the schedule has more moving parts and a handful
of transfers can show up over a season (in one 20-week test with 14 teams
and 2 byes/week, 9 total transfers across the whole season) -- still a
small fraction of games, and the app is doing a one-week lookahead every
time to keep that number down. Blackout-driven schedule tweaking can add
a few more on top of that.

## How field assignment works

There are 7 fields (9-15): 10-13 are equally desirable, and 9, 14, and 15
are the lower-quality fields. Each week, if there are more games than good
fields, the app sends the *lowest-quality-field duty owed* matchups to the
low fields -- specifically, it ranks each week's matchups by how many
low-field games the two teams involved have already racked up combined,
and puts the pairs with the fewest onto fields 9, 14, and 15 first. Every
other matchup gets one of the four good fields. Since a league of up to 6
teams (3 games/week) never needs more than the 4 good fields, low fields
only come into play once a league is big enough to need them -- and from
then on the app keeps steering low-field duty toward whoever's had the
least of it.
